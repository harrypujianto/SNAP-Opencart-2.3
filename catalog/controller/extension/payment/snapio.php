...
class ControllerExtensionPaymentSnapio extends Controller {
  ...
  public function index() {
    ...
    $data['text_loading'] = $this->language->get('text_loading');
     ...
     if (VERSION > 2.1 ) {
        ...
        return $this->load->view('default/template/payment/snapio.tpl', $data);
      }
    }
  ...
  public function process_order() {
    ...
    if ($this->config->get('config_currency') != 'IDR') {
      ...
      else if ($this->config->get('snapio_currency_conversion') > 0) {
        foreach ($item_details as &$item) {
          $item['price'] = intval($item['price']
              * $this->config->get('snapio_currency_conversion'));
        }
        unset($item);
